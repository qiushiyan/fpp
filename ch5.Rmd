...
```
 ...
 `stretch_tsibble()` (may subject to change, I havn't found a corresponding function in the **slider** package) generates multiple folds as we specified:  
...
The difference between the simple back-transformed forecast given by \@ref(eq:box-cox) and the mean given by \@ref(eq:bias-adjusted) is called the **bias**. When we use the mean, rather than the median, we say the point forecasts have been **bias-adjusted**.
