...
Surprisingly, this shed light on how we would decide the order $q$. For a MA(2) process, ACF would be 0 for $k = 3, 4, \dots$. In general, for a MA(q) process, ACF will be nonzero for $k = 1, 2, \dots,q$ and zero after $q$.
...
\end{aligned}
$$
Now everything is clear, ACF will zero, if and only if there is no single square term. That is to say, 
there is no **same** error term when we decompose $y_t$ and $y_{t-k}$. This means the earliest error in the RHD of $y_t$ is still earlier than the latest error term in that of $y_{t-k}$ 

$$
t - q > t - k \quad \rightarrow k > q 
$$

We have proved that when $k > q$, ACF would be zero and otherwise non-zero. So, a sample ACF with significant autocorrelations at lag 1 to lag q, but non-significant autocorrelations for after q indicates a possible MA(q) model.
...
y_t &=  c + \phi_1y_{t-1} + \epsilon_t  \\
    &=  c +  \phi_1(c + \phi_1y_{t-2} + \epsilon_{t-1}) + \epsilon_t  \\
    &= (c + \phi_1c)+ \phi_1^2y_{t-2} + \phi_1\epsilon_{t-1} +  + \epsilon_t \\
    &= (c+ \phi_1c + \phi_1^2c) + \phi_1^3y_{t-3} + \phi_1^2\epsilon_{t-2} + \phi_1\epsilon_{t-1} + \epsilon_t \\ 
    &=\vdots \\
    &= (c + \phi_1c + \phi_1^2c + \cdots + \phi_1^kc) + \phi_1^ky_{t-k} + \phi_1^{k-1}\epsilon_{t-(k-1)} + \dots + \phi_1\epsilon_{t-1} + \epsilon_t 
\end{split}
$$
